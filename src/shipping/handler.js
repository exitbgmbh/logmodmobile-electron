...
class ShippingHandler
{
    ...
    handleShipping = (data) => {
        ...
        restClientInstance.requestShipOut(boxIdentification, requestData).then((response) => {
            ...
            eventEmitter.emit('shipOutSucceed', {event: 'shipOutSucceed', type: 'pickBox', receiverUserId: 0, receiverLogModIdent: '', data: {boxIdent: boxIdentification, shipOutType: 'POLLING'}});

            this.shippingCounter++;
        }).catch((err) => {
            eventEmitter.emit('shipOutFailed', {event: 'shipOutFailed', type: 'pickBox', receiverUserId: 0, receiverLogModIdent: '', data: {boxIdent: boxIdentification}});
